...
class TestLeaveHelpers(TestCase):
    """Pure helper _normalize_intent (PublicCheckEmailView)."""
      # normalize staff memebers as general employees
...
class TestLeaveAPI(BaseAuthAPITestCase):
    ...
    # creates a unique ID for HR Head
    def _unique_hr(self):
...
# check whether field officer can submit leave requests successfully 
...
# check whether hr head cannot submit leave requests
...
# check hr can view all leave requests 
    ...
    def test_hr_updates_leave_approved_200(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        # check whether status is updated in db
        self.assertTrue(res.data.get('success'))
    ...
    def test_cancel_approved_future_leave_200(self):
        ...
        # sort by id dec of leave requests
