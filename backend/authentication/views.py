...
class GeneratePaymentSlipsView(APIView):
    ...
    # access only authenticated classes 
    permission_classes = (IsAuthenticated,)
    ...
    def post(self, request):
        ...
        try:
            ...
            # If month or year is not provided, default to current month and year
            month = int(month_raw) if month_raw else timezone.now().month
            year = int(year_raw) if year_raw else timezone.now().year
        except (ValueError, TypeError):# if data type or value 
            return Response({
        ...
        try:
         ...
         # create or update PaymentSlip model instances for all users 
            ...
            try:
                from system_logs.utils import log_action, get_client_ip
                #record/log an action in the system
                #  after payment slips are generated
                log_action(
                ...
                pass
            #send notification to the person who generated payments slips
            _notify_auth_users(
                [request.user],
                category='payment',
                severity='info',
...
class UploadPaymentSlipsView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # update the updated_count in the paymentslip model
...
class MyPaymentSlipsView(generics.ListAPIView):
    ...
    # to include request for serializer context to build 
    # absolute URLs
...
class AllPaymentSlipsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        # Check if user is admin or HR Head
        # hr head and admin have no payments
        if not hasattr(self.request.user, 'role') or self.request.user.role.role not in ['admin', 'hr_head']:
            return PaymentSlip.objects.none()
        # get queryset of payments, exclude hr head and admin
        ...
        return queryset
 # collects extra values and extra names using args and kwargs
    def list(self, request, *args, **kwargs):
        """Override list to wrap response in standard format"""
        queryset = self.get_queryset()
        serializer = self.get_serializer(queryset, many=True)
...
class PaymentSlipDetailView(generics.RetrieveUpdateDestroyAPIView):
    """Get, update, and delete payment slip details ( HR Head and admin can view, update)"""
    permission_classes = (IsAuthenticated,)
    serializer_class = PaymentSlipSerializer
    ...
    def get_serializer_context(self):
        """Add request to serializer context for building absolute URLs"""
        context = super().get_serializer_context()
        #give serializer access to the current request
        context['request'] = self.request
        return context
...
class CreateEmployeeRemovalRequestView(APIView):
    ...
    def post(self, request):
        ...
        # get the user id that requested to remove
        ...
        try:
            ...
            # the model that saved in database
...
class AllRemovalRequestsView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        #after checking permissions,continue the listing as normal
...
class ApproveRemovalRequestView(APIView):
    ...
    def post(self, request, request_id):
        ...
        try:
            ...
            # save the approval info to the database
